...
router.post('/', async (req, res) => {
  try {
    const { texte, cover_base64 } = req.body;
    ...
    let finalContent = "";
    if (cover_base64) {
      finalContent += `<div style="text-align:center;margin-bottom:30px;">
        <img src="${cover_base64}" style="max-width:100%;height:auto;page-break-after:always;">
      </div>`;
    }
    finalContent += contenu;

    html = html.replace("<!-- contenu injecté dynamiquement -->", finalContent);
